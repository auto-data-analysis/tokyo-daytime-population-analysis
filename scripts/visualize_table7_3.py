...
for bar in bars1:
    ...
    ax1.text(
        ...
        f'{int(width):+,}',
        va='center', ha=ha, fontsize=9, color=TEXT_COLOR,
    )
...
ax1.text(0.02, 0.02, '■ 減少', transform=ax1.transAxes,
         color=COLOR_DECREASE, fontsize=9)
ax1.text(0.12, 0.02, '■ 増加', transform=ax1.transAxes,
...
for year_val, y_pos in zip([2020, 2015], y_positions):
    ...
    for cat in CATEGORIES2:
        val = float(row[cat])
        ax2.barh(y_pos, val, left=left, height=BAR_HEIGHT,
                 color=COLORS_BREAKDOWN[cat], linewidth=0)
        left += val
    # 総数ラベル
    total = sum(float(row[cat]) for cat in CATEGORIES2)
    ax2.text(total + 200, y_pos,
             f'{int(total):,}人',
...
# 凡例
legend_handles = [
    mpatches.Patch(color=COLORS_BREAKDOWN[cat], label=cat)
    for cat in CATEGORIES2
]
ax2.legend(handles=legend_handles, loc='lower right',
           bbox_to_anchor=(1.0, -0.25), ncol=4,
