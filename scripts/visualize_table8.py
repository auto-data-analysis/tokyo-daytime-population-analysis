...
for ax, ku in zip(axes, KU_ORDER):
    ...
    for _, row in df_ku.iterrows():
        ...
        if val < 0:
            ...
            ha = 'left'
        ax.text(x_pos, row['産業名'], f'{int(val):+,}',
                va='center', ha=ha, fontsize=6.5, color=TEXT_COLOR)
...
# 色の凡例（右端パネル外側）
axes[-1].text(1.02, 0.02, '■ 減少', transform=axes[-1].transAxes,
              color=COLOR_DECREASE, fontsize=8, va='bottom')
axes[-1].text(1.02, 0.07, '■ 増加', transform=axes[-1].transAxes,
              color=COLOR_INCREASE, fontsize=8, va='bottom')
